...
def test_sudo_with_no_pass_uses_secrets_as_password():
    ...
    servers = ServerGroup(user="admin", port=22, hosts=["10.0.0.1"], password="no_pass")
    conn = _make_connection()
    progress_mock = _make_progress_mock()
    ...
    with _patch_progress(progress_mock):
        with patch("ikctl.runner.remote.SftpTransfer") as MockSftp:
            ...
            runner = RemoteRunner(
                connection_factory=lambda host: conn,
                secrets="real_secret_pass",
            )
...
def test_sudo_with_real_password_uses_servers_password():
    ...
    servers = ServerGroup(user="admin", port=22, hosts=["10.0.0.1"], password="mypassword")
    conn = _make_connection()
    progress_mock = _make_progress_mock()
...
def test_sudo_with_no_pass_and_no_secrets_uses_empty():
    """When servers.password == 'no_pass' and secrets is empty, sudo uses empty password."""
    kit = KitPipeline(
        name="mykit",
        uploads=["/local/kits/mykit/script.sh"],
        pipeline=["/local/kits/mykit/script.sh"],
