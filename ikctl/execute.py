...
class Exec:
    ...
    def run_remote(self, conn, options, path, kit, mode, password):
        ...
        if mode == "command":
            ...
            command = self.commands("cd " + path + ";" + "echo "+password+" | sudo -S bash " +
                                    kit + " " + ' '.join(options.parameter), conn.connection)

        elif options.sudo and not options.parameter:
            command = self.commands(
                "cd " + path + ";" + "echo "+password+" | sudo -S bash " + kit, conn.connection)

        elif not options.sudo and options.parameter:
            command = self.commands("cd " + path + ";" + "bash " +
                                    kit + " " + ' '.join(options.parameter), conn.connection)

        elif not options.sudo and not options.parameter:
            command = self.commands(
                "cd " + path + ";" + "bash " + kit, conn.connection)
    ...
    def run_local(self, options, path_kits, password):
        ...
        if options.sudo and options.parameter:
            command = self.commands(
                f'cd {path}; echo {password} | sudo -S bash {kit} {" ".join(options.parameter)}')

        elif options.sudo and not options.parameter:
            command = self.commands(
                f'cd {path}; echo {password} | sudo -S bash {kit}')

        elif not options.sudo and options.parameter:
            command = self.commands(
                f"cd {path}; bash {kit} {' '.join(options.parameter)}")
